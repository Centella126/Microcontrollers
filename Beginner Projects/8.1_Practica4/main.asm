...
;-------------------------------------------------------------------------------
			...
			bis.b	#BIT7, &P4DIR	; Set the LED at P1.0 as output. bic.b	#BIT0, &P1OUT	; Ensure the LED is off to start with. bic.w	#MC0|MC1, &TA0CTL	; Turn off timer TA0.
			bic.b 	#BIT7, &P4OUT ; Ensure the LED is off to start with.
			...
			bis.w	#ID0|ID1, &TA0CTL	; Dividir entre 8
...
TA0_ISR:
			...
			xor.b	#BIT7, &P4OUT	; Toggle the LED at P1.0.
			cmp	#1, r4	; Check flag to see if we need to debounce the switch.

			jnz	debonuce
			bis.b	#1, &P1IE	; Enable the interrupt on P1.1 if it isn't the
			; first time after the button was pressed.
			;#1 es lo mismo que #BIT1
...
PORT1_ISR:	; This is the ISR that gets called when
			...
			bic.w	#ID0|ID1, &TA0CTL	; Dividir entre 1
			...
			mov.w	#BIT1, r4	; This is the flag that the button was just
			; pushed.
			Reti
